<template>
  <div class="payments-page">
    ...
    <div v-if="loading && payments.length === 0" class="initial-loader">
      Loading payments...
    </div>
    ...
    <div v-else>
      ...
      <GenericTable :items="payments" :all-fields="allFields" :selected-fields="selectedFields" :next-page="nextPage"
        :searchPlaceholder="'Search payments'" :itemLabelPlural="'payments'" :isLoadingMore="isLoadingMore"
...
export default {
  ...
  setup() {
    ...
    const fetchData = async (pageToken = null) => {
      ...
      try {
        ...
        if (pageToken) {
          ...
          payments.value = []; // Reset data on initial load
        }

        const paymentsData = await getPayments(token, numericLimit, pageToken);

        if (pageToken) {
          payments.value.push(...(paymentsData.results || [])); // Append for infinite scroll
        } else {
          payments.value = paymentsData.results || []; // Replace for initial load
        }

        nextPage.value = paymentsData.nextPage || null;

        if (allFields.value.length === 0) {
          const fields = await fetchPaymentFields(token);
          allFields.value = fields;

          const preferences = await fetchPreferences("payments");
    ...
    const updatePreferences = async (updatedFields) => {
      const token = localStorage.getItem("hubspotToken");
      try {
        await savePreferences("payments", updatedFields, token);
    ...
    const handleExportAll = async () => {
      ...
      try {
        const token = localStorage.getItem("hubspotToken");
        const response = await exportAllPayments(token);
