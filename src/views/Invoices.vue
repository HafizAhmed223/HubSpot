<template>
  <div class="invoices-page">
    ...
    <div v-if="loading && invoices.length === 0" class="initial-loader">
      Loading invoices...
    </div>
    ...
    <div v-else>
      ...
      <GenericTable :items="invoices" :all-fields="allFields" :selected-fields="selectedFields" :next-page="nextPage"
        :searchPlaceholder="'Search invoices'" :itemLabelPlural="'invoices'" :isLoadingMore="isLoadingMore"
...
export default {
  ...
  setup() {
    ...
    const fetchData = async (pageToken = null) => {
      ...
      try {
        ...
        if (pageToken) {
          ...
          invoices.value = []; // Reset data on initial load
        }

        const invoicesData = await getInvoices(token, numericLimit, pageToken);

        if (pageToken) {
          invoices.value.push(...(invoicesData.results || [])); // Append for infinite scroll
        } else {
          invoices.value = invoicesData.results || []; // Replace for initial load
        }

        nextPage.value = invoicesData.nextPage || null;

        if (allFields.value.length === 0) {
          const fields = await fetchInvoiceFields(token);
          allFields.value = fields;

          const preferences = await fetchPreferences("invoices");
    ...
    const updatePreferences = async (updatedFields) => {
      const token = localStorage.getItem("hubspotToken");
      try {
        await savePreferences("invoices", updatedFields, token);
    ...
    const handleExportAll = async () => {
      ...
      try {
        const token = localStorage.getItem("hubspotToken");
        const response = await exportAllInvoices(token);
