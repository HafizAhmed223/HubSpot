<template>
  <div class="deals-page">
    ...
    <div v-if="loading && deals.length === 0" class="initial-loader">
      Loading deals...
    </div>
    ...
    <div v-else>
      ...
      <GenericTable :items="deals" :all-fields="allFields" :selected-fields="selectedFields" :next-page="nextPage"
        :searchPlaceholder="'Search deals'" :itemLabelPlural="'deals'" :isLoadingMore="isLoadingMore"
        @fetch-next-page="fetchNextPage" @update-fields="updatePreferences" @export-all="handleExportAll"
        @search-contacts="onSearchDeals" />
    </div>
  ...
  setup() {
    ...
    const fetchData = async (pageToken = null) => {
      ...
      try {
        ...
        if (pageToken) {
          deals.value.push(...(dealsData.results || []));
        } else {
          deals.value = dealsData.results || [];
        }

        nextPage.value = dealsData.nextPage || null;
        totalRecords.value = dealsData.total || 0;
      } catch (err) {
      ...
      if (allFields.value.length === 0) {
        try {
          ...
          const preferences = await fetchPreferences("deals");
    ...
    const updatePreferences = async (updatedFields) => {
      const token = localStorage.getItem("hubspotToken");
      try {
        await savePreferences("deals", updatedFields, token);
    ...
    const onSearchDeals = (searchTerm) => {
    ...
    const handleExportAll = async () => {
      ...
      try {
        const token = localStorage.getItem("hubspotToken");
        const response = await exportAllDeals(token);
