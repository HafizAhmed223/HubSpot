<template>
  <div class="products-page">
    ...
    <div v-if="loading && products.length === 0" class="initial-loader">
      Loading products...
    </div>
    ...
    <div v-else>
      ...
      <GenericTable
        ...
        @search-contacts="onSearchProducts"
      />
    </div>
  ...
  setup() {
    ...
    const fetchData = async (pageToken = null) => {
      ...
      try {
        ...
        if (pageToken) {
          products.value.push(...(productsData.results || []));
        } else {
          products.value = productsData.results || [];
        }

        nextPage.value = productsData.nextPage || null;
        totalRecords.value = productsData.total || 0;
      } catch (err) {
      ...
      if (allFields.value.length === 0) {
        try {
          const fields = await fetchProductFields(token);
          allFields.value = fields;

          const preferences = await fetchPreferences("products");
    ...
    const updatePreferences = async (updatedFields) => {
      const token = localStorage.getItem("hubspotToken");
      try {
        await savePreferences("products", updatedFields, token);
    ...
    const handleExportAll = async () => {
      ...
      try {
        const token = localStorage.getItem("hubspotToken");
        const response = await exportAllProducts(token);
